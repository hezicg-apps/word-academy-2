...
style.innerHTML = `
    ...
    .game-btn { 
        ...
        border: 2px solid #facc15; border-radius: 1.5rem; 
    ...
    /* מקלדת קוד סודי - גובה אחיד וסידור צדדים */
    .keyboard-fixed { position: fixed; bottom: 0; left: 0; right: 0; padding: 10px; background: #e2e8f0; direction: ltr; }
    ...
    .key { flex: 1; height: 50px; display: flex; align-items: center; justify-content: center; background: white; border-radius: 6px; font-weight: 800; color: #000; font-size: 1rem; }
    .key-enter { background: #22c55e; color: white; flex: 2; }
    .key-del { background: #94a3b8; color: white; flex: 2; }

    /* משחק הזיכרון - טקסט גדול ללא שבירה */
    .mem-text { 
        font-size: 1.4rem; font-weight: 900; text-align: center; 
        white-space: nowrap; max-width: 90%; 
    }
    @media (max-width: 400px) { .mem-text { font-size: 1.1rem; } }

    .card-container { perspective: 1000px; width: 100%; aspect-ratio: 1/1.2; }
    .card-inner { position: relative; width: 100%; height: 100%; transition: 0.6s; transform-style: preserve-3d; cursor: pointer; }
    .is-flipped { transform: rotateY(180deg); }
    .card-face { position: absolute; inset: 0; backface-visibility: hidden; border-radius: 1rem; display: flex; flex-direction: column; align-items: center; justify-content: center; border: 2px solid #e2e8f0; background: white; color: #000; overflow: hidden; }
...
function shareList() {
    const url = getShareUrl();
    const text = `הנה רשימת מילים חדשה באנגלית: "${state.listName}" 📝\nמוזמנים ללמוד ולשחק: `;
    window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(text + url)}`, '_blank');
}

// --- רינדור והגדרות מסכים ---
function render() {
    ...
    main.className = "p-4 flex-1 flex flex-col max-w-2xl mx-auto w-full";
    ...
    (screens[state.screen] || renderInput)(main);
}

function renderInput(container) {
    container.innerHTML = `
        <div class="py-6 space-y-6 text-center welcome-box rounded-3xl p-4">
            <h2 class="text-3xl font-bold">הזנת מילים</h2>
            <textarea id="wordInput" class="w-full h-48 p-4 border-2 rounded-2xl text-right bg-white font-bold text-black" placeholder="שם היחידה\nEnglish - עברית"></textarea>
            <div class="grid gap-3">
                <button onclick="processInput()" class="bg-blue-600 text-white p-5 rounded-xl font-bold w-full shadow-lg text-xl">מתחילים ללמוד 🚀</button>
            </div>
        </div>`;
}
...
function checkQuizAnswer(selected, correct, btn) {
    ...
    setTimeout(() => {
        ...
        else { state.lastScore = Math.round((state.correctAnswers/state.words.length)*100); state.screen = 'report'; saveToLocal(); render(); }
    }, 1200);
}
...
function renderMenu(container) {
    ...
    container.innerHTML = `
        ...
        <div class="grid gap-2 w-full px-4">
            ...
            <button onclick="shareList()" class="mt-4 p-4 border-2 border-dashed border-slate-300 rounded-2xl font-bold text-slate-500">שתף רשימה זו לחברים 🔗</button>
        </div>
        ${locked ? `<div class="mt-6 text-center px-6"><p class="text-red-500 font-bold bg-red-50 p-3 rounded-xl border border-red-100">המשחקים נעולים! השג לפחות 70% במבחן כדי לפתוח אותם.</p></div>` : ''}
    ...
    container.innerHTML = `<div class="w-full pb-64">
        ...
        <div class="grid gap-2 px-4" style="direction: ltr; grid-template-columns: repeat(${len}, 45px); justify-content: center;">
            ${Array(5).fill(0).map((_, rowIndex) => {
                const guess = q.guesses[rowIndex];
                return Array(len).fill(0).map((_, colIndex) => {
                    let char = ''; let bg = 'bg-white border-2 border-slate-200'; let txt = 'text-black';
                    if (guess) {
                        char = guess[colIndex];
                        if (q.target[colIndex] === char) bg = 'bg-[#ffd700] border-[#ffd700]';
                        else if (q.target.includes(char)) bg = 'bg-[#c0c0c0] border-[#c0c0c0]';
                        else bg = 'bg-slate-200 border-slate-200';
                        txt = 'text-black';
                    } else if (rowIndex === q.guesses.length) {
                        char = q.currentGuess[colIndex] || '';
                        if (char) bg = 'border-blue-400';
                    }
                    return `<div class="w-11 h-14 flex items-center justify-center rounded-lg text-2xl font-black shadow-sm ${bg} ${txt}">${char}</div>`;
                }).join('');
            }).join('')}
        </div>
...
function renderWQSummary() {
    return `<div class="fixed inset-0 bg-black/90 flex items-center justify-center p-6 z-[60] text-center">
        <div class="p-8 rounded-3xl border-4 border-yellow-400 bg-zinc-900">
            ...
            ${g.cards.map((c, i) => `
                <div class="card-container" onclick="flipMem(${i})">
                    <div class="card-inner ${g.flipped.includes(i) || c.match ? 'is-flipped' : ''}">
                        <div class="card-face bg-blue-600 text-white text-4xl font-bold border-none">?</div>
                        <div class="card-face card-back p-2">
                            <span class="mem-text text-black">${c.text}</span>
                        </div>
                    </div>
                </div>
            `).join('')}
...
function renderMemSummary() {
    ...
    return `<div class="fixed inset-0 bg-black/90 flex items-center justify-center p-6 z-[90] text-center">
        <div class="p-10 rounded-3xl border-4 border-yellow-400 bg-zinc-900">
            <h2 class="text-3xl font-black text-white">זיכרון מעולה! 🧠</h2>
            <p class="text-yellow-400 mt-2 font-bold text-xl">פתרת ב-${g.steps} צעדים.</p>
            <p class="text-white mb-6">חושב שאפשר בפחות? נסה שוב!</p>
            <button onclick="initMemGame()" class="bg-green-600 text-white px-8 py-3 rounded-xl font-bold w-full mb-2">נסה לשפר תוצאה 🔄</button>
            <button onclick="state.screen='menu'; render()" class="bg-blue-600 text-white px-8 py-3 rounded-xl font-bold w-full">חזרה</button>
        </div>
    </div>`;
}
...
function renderC4(container) {
    ...
    container.innerHTML = `<div class="flex flex-col items-center gap-4 w-full px-2">
        <div class="w-full flex justify-between items-center max-w-[280px] font-bold">
            <button onclick="prepC4Q()" class="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm shadow-md">שאלה לאסימון 🎲</button>
            <div class="flex items-center gap-2">תור: <div class="w-6 h-6 rounded-full ${g.turn===1?'bg-[#ef4444]':'bg-[#facc15]'}"></div></div>
        </div>
        <div class="c4-board">${g.board.map((row) => `<div class="c4-row">${row.map((cell, c) => `<div onclick="dropToken(${c})" class="c4-cell">${cell ? `<div class="token ${cell===1?'bg-[#ef4444]':'bg-[#facc15]'}"></div>` : ''}</div>`).join('')}</div>`).join('')}</div>
...
window.onload = () => {
    ...
    if (saved) {
        ...
        state.listName = parsed.listName;
        state.lastScore = parsed.lastScore;
        state.nightMode = parsed.nightMode;
        if(state.nightMode) document.body.classList.add('dark');
        if(state.words.length) state.screen = state.lastScore >= 70 ? 'menu' : 'flashcards';
    }
    render();
};
