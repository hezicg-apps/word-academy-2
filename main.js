    ...
    container.innerHTML = `
        <div class="p-6 space-y-6 text-center animate-fade-in">
            ...
            <textarea id="wordInput" 
                class="w-full h-64 p-6 border-4 rounded-[2rem] outline-none font-bold text-right text-xl shadow-inner transition-colors
                ${isDark ? 'bg-slate-800 border-slate-700 text-white placeholder-slate-500' : 'bg-white border-blue-50 text-slate-800'}" 
                placeholder="שם הרשימה\ndog - כלב\ncat - חתול"></textarea>
            <button onclick="saveNewList()" class="w-full bg-blue-600 text-white py-6 rounded-3xl font-black text-2xl shadow-xl">המשך לכרטיסיות ✨</button>
        </div>
    `;
}
...
function renderWelcomeScreen(container) {
    container.innerHTML = `<div class="p-10 text-center"><h1 class="text-5xl font-black text-blue-600 mb-8">${state.listName}</h1><button onclick="state.screen='flashcards'; render();" class="bg-blue-600 text-white px-12 py-6 rounded-3xl font-black text-3xl shadow-xl">קדימה! 🚀</button></div>`;
}
    ...
    container.innerHTML = `
        <div class="p-4 space-y-8 text-center animate-fade-in">
            ...
            <div class="flex gap-4 max-w-md mx-auto">
                <button onclick="nextWord(false)" class="flex-1 bg-slate-200 py-6 rounded-3xl font-black text-xl text-slate-700">⌛ עוד לא</button>
    ...
    container.innerHTML = `
        <div class="p-6 text-center space-y-10 animate-fade-in">
            ...
            <div class="grid gap-4 max-w-md mx-auto" id="quiz-options">
                ${choices.map(c => `<button onclick="checkQuiz('${c}', '${q.heb}', this)" class="bg-white p-6 rounded-3xl font-black text-2xl border-4 border-slate-50 shadow-md text-slate-800">${c}</button>`).join('')}
            </div>
        </div>
    `;
}
...
function renderReportScreen(container) {
    container.innerHTML = `
        <div class="p-6 text-center space-y-8 animate-fade-in">
            <div class="bg-white p-10 rounded-[3.5rem] shadow-2xl border-b-[12px] border-yellow-400 text-slate-800">
                ...
                <div class="flex flex-wrap justify-center gap-4">
                    <button onclick="state.quizIndex=0; state.correctAnswers=0; state.screen='quiz'; render();" class="bg-orange-500 text-white px-8 py-4 rounded-2xl font-black text-xl">🔄 תרגול חוזר</button>
                    <button onclick="openShareModal(true)" class="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black text-xl">🔗 שיתוף תוצאה</button>
                </div>
            </div>
            ...
            <button onclick="state.screen='menu'; render();" class="w-full bg-green-500 text-white py-8 rounded-[2.5rem] font-black text-3xl shadow-xl">בואו נשחק! 🎮</button>
        </div>
    `;
}
...
function renderMenuScreen(container) {
    container.innerHTML = `
        <div class="p-6 space-y-6 animate-fade-in text-center">
            <h1 class="text-5xl font-black text-blue-600 mb-10 italic">Word Games</h1>
            <div class="grid gap-6 max-w-md mx-auto">
                <button onclick="initMemoryGame()" class="group bg-white p-8 rounded-[2.5rem] shadow-xl border-b-8 border-blue-400 flex items-center gap-6 hover:scale-105 transition-all text-right text-slate-800">
                    <span class="text-6xl">🧠</span>
                    <div><div class="font-black text-2xl">משחק הזיכרון</div><div class="text-slate-400 text-sm">מצאו את הזוגות</div></div>
                </button>
                <button onclick="initConnect4()" class="group bg-white p-8 rounded-[2.5rem] shadow-xl border-b-8 border-red-400 flex items-center gap-6 hover:scale-105 transition-all text-right text-slate-800">
                    <span class="text-6xl">🔴</span>
                    <div><div class="font-black text-2xl">4 בשורה</div><div class="text-slate-400 text-sm">נצחו את המחשב</div></div>
                </button>
                <button onclick="initWordQuest()" class="group bg-white p-8 rounded-[2.5rem] shadow-xl border-b-8 border-green-400 flex items-center gap-6 hover:scale-105 transition-all text-right text-slate-800">
                    <span class="text-6xl">🔍</span>
                    <div><div class="font-black text-2xl">הקוד הסודי</div><div class="text-slate-400 text-sm">נחשו את המילה</div></div>
                </button>
            </div>
        </div>
    `;
                    ...
                    return `
                        <div onclick="handleMemoryClick(${i})" class="aspect-square perspective-1000 cursor-pointer h-32 md:h-40">
                            <div class="relative w-full h-full transition-all duration-500 preserve-3d ${flipped ? 'rotate-y-180' : ''}">
                                <div class="absolute inset-0 bg-blue-600 rounded-[2rem] flex items-center justify-center backface-hidden border-b-8 border-blue-800">
                                    <span class="text-5xl text-white">❓</span>
                                </div>
                                <div class="absolute inset-0 bg-white border-4 border-blue-400 rounded-[2rem] flex items-center justify-center p-2 backface-hidden rotate-y-180">
                                    <span class="font-black text-blue-600 leading-tight" style="font-size: clamp(16px, 4vw, 28px);">${card.content}</span>
                                </div>
                            </div>
                        </div>`;
                }).join('')}
...
function handleMemoryClick(i) {
    ...
    if (g.flipped.length === 2) {
        ...
        if (g.cards[f].id === g.cards[s].id && g.cards[f].type !== g.cards[s].type) {
            ...
            setTimeout(() => { 
                speak(eng); g.cards[f].matched = true; g.cards[s].matched = true;
...
function renderConnect4(container) {
    ...
    if (!g.mode) {
        container.innerHTML = `<div class="p-10 text-center"><div class="bg-white p-12 rounded-[4rem] shadow-2xl border-4 border-blue-100 max-w-md mx-auto"><h2 class="text-5xl font-black text-blue-600 mb-10 text-slate-800">4 בשורה</h2><button onclick="state.connect4.mode='pvp'; render();" class="w-full bg-blue-600 text-white py-6 rounded-3xl font-black text-2xl shadow-lg mb-6">משחק זוגי</button><button onclick="state.connect4.mode='pve'; render();" class="w-full bg-orange-600 text-white py-6 rounded-3xl font-black text-2xl shadow-lg">נגד המחשב</button></div></div>`;
        return;
    }
    container.innerHTML = `
        <div class="p-4 flex flex-col items-center">
            <div class="w-full max-w-3xl flex justify-between items-center bg-slate-800 p-6 rounded-[2rem] text-white mb-8">
                ...
                <button onclick="state.connect4.isAnswering=true; prepC4Q(); render();" ${g.canDrop || g.animatingRow!==-1 ? 'disabled' : ''} class="bg-blue-600 px-8 py-3 rounded-2xl font-black text-xl disabled:opacity-30">שאלה לאסימון ❓</button>
            </div>
            <div class="bg-blue-700 p-6 rounded-[3rem] shadow-2xl grid grid-cols-7 gap-4 w-full max-w-3xl border-b-[16px] border-blue-900">
                ${g.board.map((row, r) => row.map((cell, c) => {
                    ...
                    return `<div onclick="handleC4Drop(${c})" class="aspect-square bg-blue-900 rounded-full flex items-center justify-center shadow-inner border-4 border-blue-800 cursor-pointer">
                        ${val === 1 ? '<div class="w-4/5 h-4/5 bg-red-500 rounded-full border-4 border-red-400 animate-pop"></div>' : 
                          val === 2 ? '<div class="w-4/5 h-4/5 bg-yellow-400 rounded-full border-4 border-yellow-300 animate-pop"></div>' : ''}
                    </div>`;
                }).join('')).join('')}
            </div>
        </div>`;
...
function renderC4Modal(container) {
    ...
    modal.innerHTML = `<div class="bg-white p-10 rounded-[3.5rem] w-full max-w-md text-center text-slate-800">
...
async function handleC4Drop(col) {
    ...
    if (checkC4Win(targetRow, col, g.turn)) { 
        triggerConfetti(); 
        setTimeout(() => { alert(`ניצחון!`); initConnect4(); }, 500); 
...
function openShareModal(isResult = false) {
    ...
    const modal = document.createElement('div');
    modal.className = "fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[200] p-4";
    modal.innerHTML = `<div class="bg-white rounded-[3rem] w-full max-w-sm overflow-hidden shadow-2xl"><div class="p-6 border-b flex justify-between items-center bg-slate-50"><h3 class="text-2xl font-black text-blue-600">שיתוף</h3><button onclick="this.closest('.fixed').remove()" class="text-slate-400 text-3xl">✕</button></div><div class="p-6 space-y-4"><a href="https://wa.me/?text=${encodeURIComponent(text + ' ' + shareUrl)}" target="_blank" class="flex items-center gap-4 p-4 border-2 border-green-100 rounded-2xl hover:bg-green-50 text-slate-800"><img src="https://upload.wikimedia.org/wikipedia/commons/6/6b/WhatsApp.svg" class="w-10 h-10"><span class="font-bold text-lg">WhatsApp</span></a><a href="mailto:?subject=בואו ללמוד אנגלית&body=${encodeURIComponent(text + ' ' + shareUrl)}" class="flex items-center gap-4 p-4 border-2 border-red-100 rounded-2xl hover:bg-red-50 text-slate-800"><img src="https://upload.wikimedia.org/wikipedia/commons/7/7e/Gmail_icon_%282020%29.svg" class="w-10 h-10"><span class="font-bold text-lg">Gmail / Email</span></a><button onclick="copyToClipboard(this, '${shareUrl}')" class="w-full flex items-center gap-4 p-4 border-2 border-blue-100 rounded-2xl hover:bg-blue-50 text-slate-800"><img src="https://www.svgrepo.com/show/474773/copy.svg" class="w-10 h-10"><span class="font-bold text-lg"><span>העתק קישור</span></span></button></div></div>`;
...
async function handleC4Drop(col) {
    ...
    let targetRow = -1;
    for (let r = 5; r >= 0; r--) {
        if (!g.board[r][col]) { targetRow = r; break; }
    }
    if (targetRow === -1) return;
    ...
    g.board[targetRow][col] = g.turn;
    g.animatingRow = -1;

    if (checkC4Win(targetRow, col, g.turn)) {
        triggerConfetti();
        setTimeout(() => {
            alert(`ניצחון ל${g.turn === 1 ? 'אדום' : 'צהוב'}!`);
            initConnect4();
        }, 500);
    } else {
        g.turn = g.turn === 1 ? 2 : 1;
        ...
        // אם זה מצב נגד המחשב ותור המחשב (צהוב)
        if (g.mode === 'pve' && g.turn === 2) {
...
function makeCPUMove() {
    ...
    let validCols = [];
    for (let c = 0; c < 7; c++) {
        if (!g.board[0][c]) validCols.push(c);
    }
    if (validCols.length > 0) {
        // המחשב בוחר עמודה אקראית, אבל אפשר להוסיף כאן לוגיקה חכמה בעתיד
        const randomCol = validCols[Math.floor(Math.random() * validCols.length)];
        state.connect4.canDrop = true; // מאפשר למחשב "להפיל"
        handleC4Drop(randomCol);
    }
}

// --- פונקציות תשתית (URL וטעינה) ---
