...
// --- CSS דינמי לתיקון האנימציות ומצב לילה ---
...
function renderInputScreen(container) {
    container.innerHTML = `
        <div class="space-y-6 text-center animate-fade-in">
            ...
            <button onclick="saveNewList()" class="w-full bg-blue-600 text-white py-5 rounded-3xl font-black text-xl shadow-lg">המשך ללמידה ✨</button>
        </div>
    `;
}
    ...
    container.innerHTML = `
        <div class="flex-1 flex flex-col items-center justify-center space-y-6 text-center h-full">
            ...
            <div onclick="learningState.isFlipped = !learningState.isFlipped; render();" class="relative w-full aspect-video max-w-md perspective-1000 cursor-pointer">
                <div class="w-full h-full preserve-3d ${learningState.isFlipped ? 'rotate-y-180' : ''}">
                    <div class="absolute inset-0 card-face bg-white border-8 border-blue-50 rounded-[3rem] flex flex-col items-center justify-center shadow-2xl backface-hidden">
                        ...
                        <button onclick="event.stopPropagation(); speak('${word.eng}')" class="mt-4 text-3xl p-3 bg-blue-50 rounded-full">🔊</button>
                    </div>
            ...
            <div class="flex gap-4 w-full max-w-md">
                <button onclick="nextWord(false)" class="flex-1 bg-slate-200 py-5 rounded-3xl font-black text-slate-700 text-xl"> עוד לא</button>
                <button onclick="nextWord(true)" class="flex-1 bg-green-500 text-white py-5 rounded-3xl font-black text-xl shadow-lg"> יודע</button>
            </div>
        </div>
    `;
}
...
function renderQuizScreen(container) {
    ...
    let choices = [...state.words].sort(() => 0.5 - Math.random()).slice(0, 4).map(w => w.heb);
    ...
    container.innerHTML = `
        <div class="flex-1 flex flex-col items-center justify-center space-y-4 text-center">
            ...
            <div class="grid gap-3 w-full max-w-md">
                ${choices.map(c => `<button onclick="checkQuiz('${c}', '${q.heb}', this)" class="bg-white p-5 rounded-2xl font-black text-xl border-4 border-slate-50 shadow-sm text-slate-800">${c}</button>`).join('')}
            </div>
        </div>
    `;
}
        ...
        <div class="p-6 text-center space-y-8 animate-fade-in">
            <div class="bg-white p-10 rounded-[3rem] shadow-2xl">
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <button onclick="openShareModal(false)" class="bg-slate-700 text-white py-4 rounded-2xl font-bold">שיתוף הרשימה לתלמידים 🔗</button>
                </div>
            </div>
            <button onclick="state.screen='menu'; render();" class="w-full bg-green-500 text-white py-6 rounded-3xl font-black text-3xl shadow-xl">בואו נשחק! 🎮</button>
        </div>
    `;
}
        ...
        <div class="p-4 space-y-4 text-center h-full flex flex-col justify-center">
            ...
            <div class="grid gap-4 max-w-md mx-auto w-full">
                <button onclick="initMemoryGame()" class="bg-white p-6 rounded-3xl shadow-lg border-b-8 border-blue-400 flex items-center gap-4 text-right">
                    ...
                    <div><div class="font-black text-xl">משחק הזיכרון</div></div>
                </button>
                <button onclick="initConnect4()" class="bg-white p-6 rounded-3xl shadow-lg border-b-8 border-red-400 flex items-center gap-4 text-right">
                    <span class="text-5xl">🔴</span>
                    <div><div class="font-black text-xl">4 בשורה</div></div>
                </button>
                <button onclick="initWordQuest()" class="bg-white p-6 rounded-3xl shadow-lg border-b-8 border-green-400 flex items-center gap-4 text-right">
                    <span class="text-5xl">🔍</span>
                    <div><div class="font-black text-xl">הקוד הסודי</div></div>
                </button>
            </div>
...
function renderWordQuest(container) {
    ...
    container.innerHTML = `
        <div class="flex-1 flex flex-col items-center justify-between p-2">
            <div class="flex justify-between w-full px-4"><button onclick="state.screen='menu'; render();">⬅️ חזור</button><span class="font-black">הקוד הסודי</span></div>
            <div class="flex items-center gap-3 bg-blue-50 p-3 rounded-2xl">
                <span class="text-xl font-bold">רמז: ${q.heb}</span>
                <button onclick="speak('${q.target}')" class="text-2xl">🔊</button>
            </div>
                ...
                ${Array(6).fill(null).map((_, i) => {
                    ...
                    return `<div class="flex gap-1">
                        ${Array(q.target.length).fill(null).map((_, j) => {
                            ...
                            if (q.guesses[i]) {
                                if (q.target[j] === char) style = "bg-green-500 text-white border-green-600";
                                else if (q.target.includes(char)) style = "bg-yellow-400 text-white border-yellow-500";
                                else style = "bg-slate-400 text-white border-slate-500";
                            }
                            return `<div class="w-10 h-12 rounded-lg flex items-center justify-center font-black text-xl ${style}">${char}</div>`;
                        }).join('')}
                    </div>`;
                }).join('')}
            </div>
            ...
            ${q.isGameOver ? `<div class="p-4 bg-white rounded-2xl shadow-xl w-full max-w-xs text-center">
                <h3 class="font-bold">המילה הייתה: ${q.target}</h3>
...
function renderConnect4(container) {
    ...
    container.innerHTML = `
        <div class="flex flex-col items-center p-2 h-full justify-between">
            ...
            <div class="grid grid-cols-7 gap-1 w-full max-w-[340px] mb-2">
                ${[1,2,3,4,5,6,7].map(n => `<div class="text-center font-bold text-slate-400">▼<br>${n}</div>`).join('')}
            </div>

            <div class="bg-blue-700 p-2 rounded-3xl shadow-xl grid grid-cols-7 gap-1 w-full max-w-[340px] relative">
                ${g.board.map((row, r) => row.map((cell, c) => {
                    ...
                    return `<div onclick="handleC4Drop(${c})" class="aspect-square bg-blue-900 rounded-full flex items-center justify-center border-2 border-blue-800">
                        ${val === 1 ? '<div class="w-4/5 h-4/5 bg-red-500 rounded-full border-2 border-red-400 animate-pop"></div>' : 
                          val === 2 ? '<div class="w-4/5 h-4/5 bg-yellow-400 rounded-full border-2 border-yellow-300 animate-pop"></div>' : ''}
                    </div>`;
                }).join('')).join('')}
            </div>
...
function renderC4Modal() {
    ...
    return `<div class="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-4">
        <div class="bg-white p-8 rounded-3xl w-full max-w-xs text-center">
            <div class="flex flex-col items-center gap-2 mb-6 text-slate-800">
                <span class="text-4xl font-black">${q.eng}</span>
                <button onclick="speak('${q.eng}')" class="text-2xl">🔊</button>
            </div>
            <div class="grid gap-2">
                ${q.choices.map(c => `<button onclick="checkC4Ans('${c}', this)" class="w-full py-4 border-2 rounded-xl font-bold text-slate-800">${c}</button>`).join('')}
            </div>
        </div>
    </div>`;
}
...
function openShareModal(isResult, score = 0) {
    ...
    modal.innerHTML = `<div class="bg-white rounded-3xl w-full max-w-sm p-6 text-slate-800">
        <h3 class="text-xl font-bold mb-4">שיתוף</h3>
        <div class="grid gap-3">
            <a href="https://wa.me/?text=${encodeURIComponent(text + ' ' + shareUrl)}" target="_blank" class="p-4 border-2 rounded-2xl flex items-center gap-3">🟢 WhatsApp</a>
            <button onclick="navigator.clipboard.writeText('${shareUrl}'); alert('הועתק!')" class="p-4 border-2 rounded-2xl flex items-center gap-3">📋 העתק קישור</button>
